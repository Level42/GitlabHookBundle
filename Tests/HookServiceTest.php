...
class HookServiceTest extends TestCase
{
    ...
    public function testAnalyseHook()
    {
        ...
        $this
                ->assertEquals("95790bf891e76fee5e1747ab589903a6a1f80f22",
                        $hook->getBefore());
        $this
                ->assertEquals("da1560886d4f094c3e6c9ef40349f7d38b5d27d7",
                        $hook->getAfter());
        ...
        $this
                ->assertEquals("git@localhost:diaspora.git",
                        $hook->getRepository()->getUrl());
        $this->assertEquals("Diaspora", $hook->getRepository()->getName());
        $this->assertEquals("", $hook->getRepository()->getDescription());
        $this
                ->assertEquals("http://localhost/diaspora",
                        $hook->getRepository()->getHomepage());
        ...
        /* @var $commit1 Commit */
        $this
                ->assertEquals("b6568db1bc1dcd7f8b4d5a946b0b91f9dacd7327",
                        $commit1->getId());
        $this
                ->assertEquals("Update Catalan translation to e38cb41.",
                        $commit1->getMessage());
        $this
                ->assertEquals("2011-12-12T14:27:31+02:00",
        ...
        $this
                ->assertEquals("jordi@softcatala.org",
                        $commit1->getAuthor()->getEmail());
